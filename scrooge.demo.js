...
{
  this.lru_omed = this.lru_omed.split('').reverse().join('');
  if(window.location.hostname != this.lru_omed.replace('1', 'a').replace('2', 'e').replace('3', 'i').replace('4', 'o').replace('|', '.').replace('#', '3'))
  {
    alert('You are using the demo script for ScroogeJS, please download the full script from codecanyon.net or contact the developer at me@mario.ec.');
    return;
  }
...
domReady(function()
{
  Scrooge.lru_omed = 'c2|43r1m';
  //Scrooge.lru_omed = 'm4c|ot1vn2.#s';
	Scrooge.start();
});
